...
fun LlamaConfig.toIni(): String = writeIni(
    linkedMapOf(
        "server" to linkedMapOf(
            ...
            "jinja" to jinja.toString(),
            "reasoning_budget" to reasoningBudget.toString(),
...
fun llamaConfigFromIni(text: String): LlamaConfig {
    ...
    val d = LlamaConfig()
    fun Map<String, String>.getAny(vararg keys: String): String? = keys.firstNotNullOfOrNull { this[it] }
    return LlamaConfig(
        host = s["host"] ?: d.host,
        ...
        jinja = s["jinja"]?.toBooleanStrictOrNull() ?: d.jinja,
        reasoningBudget = s.getAny("reasoning_budget", "reasoning-budget")?.toIntOrNull() ?: d.reasoningBudget,
        cacheTypeK = s.getAny("cache_type_k", "cache-type-k") ?: d.cacheTypeK,
        cacheTypeV = s.getAny("cache_type_v", "cache-type-v") ?: d.cacheTypeV,
