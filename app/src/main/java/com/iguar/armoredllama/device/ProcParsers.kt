...
/**
 * Pure parsers for the Linux `/proc` + `sysfs` text formats behind live telemetry
 * (CPU %, per-core MHz, memory, temperature). Kept free of any Android dependency so they can be
 * unit-tested on the host JVM; [DeviceTelemetry] does the file IO.
 */
