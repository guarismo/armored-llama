...
/**
 * Real device telemetry for WIRE THIS #3–5: CPU %, per-core MHz, memory and temperature,
 * read from `/proc` and `sysfs`. Every accessor returns null/0 when the source is
 * unreadable so callers can fall back to mock data. CPU % is stateful (needs two samples).
 */
